...
const FeedbackFormHof = () => {
  ...
  const SubmitReport = () => {
    ...
    if (!formData["HOF's comments"])
      return setMessage({ ...message, error: "Comment is empty" });
    if (!formData["report_ci"])
      return setMessage({ ...message, error: "CI must be selected" });
    if (!hof?.service_id) {
  ...
  const GetPrevReport = () => {
    ...
    axios
      .get(
        `/hof/getfeedback?service_id=${selectedReport?.service_id}&hof_service_id=${hof?.service_id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
      ...
      <div class="row align-items-start">
        <div class="col-md-2 col-auto min-vh-100" id="sidebarfeedback-md">
          ...
          <select
            ...
            value={reports ? reports.indexOf(selectedReport) : -1}
            onChange={(e) => {
              setPrevReport([]);
              setSelectedReport(reports[e.target.value]);
              setFormData({
                ...formData,
                "HOF's comments": reports[e.target.value].hof_comments,
              });
            }}
          >
        ...
        <div class="col-md-10 offset-md-2 col-xs-12">
          ...
          <div class="mb-2">Academics : {selectedReport["academics"]}</div>
          <div class="mb-2">Projects : {selectedReport["projects"]}</div>
          <div class="mb-2">Sick Report : {selectedReport["sick_report"]}</div>
          <div class="mb-2">OLQ : {selectedReport["olq"]}</div>
          <div class="mb-2">Games : {selectedReport["games"]}</div>
          <div class="mb-2">Cultural : {selectedReport["cultural"]}</div>
          <div class="mb-2">Financial : {selectedReport["financial"]}</div>
          <div class="mb-2">Personal : {selectedReport["personal"]}</div>
          <div class="mb-2">
            DS's comments : {selectedReport["ds_comments"]}
          </div>
          {selectedReport["ci_comments"]?.length > 0 && (
            <div class="mb-2">
              CI's comments : {selectedReport["ci_comments"]}
            </div>
          )}
          <div class="input-group mb-2">
            ...
            <textarea
              class="form-control"
              disabled={selectedReport["ci_comments"]?.length > 0}
              value={formData["HOF's comments"]}
              name="HOF's comments"
          ...
          <div className="input-group mb-2">
            <select
              class="form-select"
              disabled={selectedReport["ci_comments"]?.length > 0}
              aria-label="Select CI"
              name="report_ci"
              onChange={handleChange}
              value={formData.report_ci}
            >
...
export default FeedbackFormHof;
