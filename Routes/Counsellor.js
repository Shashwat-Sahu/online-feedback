...
router.delete("/delete", verifyToken, (req, res) => {
  ...
  Counsellor.findOneAndDelete({ service_id }).then((data) => {
    ...
    if (data?.counselee_list.length > 0) {
      Counselee.deleteMany({ service_id: { $in: data.counselee_list } }).then(
        (data) => {
...
router.put("/addCounseleeList", verifyToken, async (req, res) => {
  ...
  Counsellor.findOne({ service_id: counsellor_service_id }).then((data) => {
    if (!data) return res.status(404).json({ error: "Counsellor not found" });
  });
  var promises = counselee_list.map((element) => {
    const { name, rank, service_id } = element;
    return Counsellor.findOne({ counselee_list: { $in: [service_id] } }).then(
      (data) => {
        if (data) return service_id;
      },
    );
  ...
  Promise.all(promises).then(async (result) => {
    ...
    if (alreadyExist.length > 0)
      return res
        .status(422)
        .json({ error: "Already Existing: " + alreadyExist.join(", ") });
    else {
